...
Plex has no password endpoint a third party may use: Tindarr brokers a **PIN**. It
creates one on plex.tv, the user approves it in their browser, and the PIN then carries
a token. Tindarr uses that token once — to read the account and the servers it may
reach — then deletes the plex.tv device it created, so no live "Tindarr" device stays
in the user's account. The only Plex token Tindarr stores is the owner token of the
...
from tindarr.ports.media_server import MediaUser, normalize_server_id, normalize_user_id
from tindarr.ports.problems import plex_tv_unreachable
