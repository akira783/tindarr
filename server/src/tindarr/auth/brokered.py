...
poll until they have approved it somewhere else. Tindarr stands in the middle so the
client never touches plex.tv or the media server itself, and never sees the upstream
credential — only a handle (``tindarr.auth.handles``).
...
- **Quick Connect.** Jellyfin opens the user's session the moment they approve the
  code, not when Tindarr collects it. Collecting therefore also ends that session, and
...
from tindarr.auth.handles import Binding, HandlePurpose, HandleRegistry
from tindarr.auth.signin import Caller, SignInService
from tindarr.core.errors import PendingError, ProblemError
from tindarr.ports.media_server import MediaUser, ServerIdentity
from tindarr.ports.plextv import PlexTv, as_media_user, find_server
...
class PlexPinFlow:
    ...
    async def _account_for(self, token: str, client_id: str, machine_id: str) -> MediaUser:
        ...
        # The device goes whatever the answer is: Tindarr keeps no Plex sign-in token,
...
class QuickConnectFlow:
    ...
    async def sweep(self) -> int:
        ...
        leave a signed-in "Tindarr server" device behind. Collecting it once logs it
