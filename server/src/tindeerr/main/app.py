...
from tindeerr.auth.mediaserver import MediaServerConnector, OwnerTokenHandles
...
def no_media_server_adapter(connection: MediaServerConnection) -> MediaServer:
    """Stand-in factory until the media server adapters arrive (step 2b).

    It fails the way an unusable connector does, so the setup wizard reports it instead
    of the server crashing.
    """
    raise ProblemError(
        502,
        "connector_unexpected_response",
        f"This build has no {connection.kind} adapter yet.",
    )
...
class Wiring:
    ...
    #: Builds the media server adapter for a stored connector (step 2b).
    media_servers: MediaServerFactory = no_media_server_adapter
    #: The Plex PIN handles of purpose ``owner_token`` (step 2b).
...
async def start(
    ...
    connector = MediaServerConnector(engine, settings, wiring.media_servers, wiring.owner_tokens)
    ...
    return Runtime(engine=engine, services=services, jobs=(purge_job(sessions),))
