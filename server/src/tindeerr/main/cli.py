...
``tindeerr [serve]`` runs the server, ``tindeerr healthcheck`` probes it, and
``tindeerr media-server reset`` is the host-side recovery path when the media server was
...
def reset_media_server(config: ServerConfig, *, confirmed: bool) -> int:
    ...
    if not confirmed and not _confirmed_on_the_terminal():
        logger.error(
            "nothing was changed. Run 'tindeerr media-server reset --yes' to confirm: it "
