...
Everything Jellyfin shares with Emby is in ``tindeerr.adapters.mediabrowser``. What is
...
- **Approval creates the session, not collection.** Jellyfin opens the user's session
  ...
  approvals nobody came back for (``tindeerr.auth.handles``).
"""
...
from tindeerr.adapters.http import RemoteCallError, read_json, read_mapping
from tindeerr.adapters.mediabrowser import MediaBrowserServer
from tindeerr.ports import problems
from tindeerr.ports.media_server import MediaServerKind, MediaUser, QuickConnectStart
