...
- **Nothing here logs a URL.** Quick Connect's secret travels in a query string
  ...
  (``tindeerr.core.logs.quiet_noisy_libraries``) and adapters log their own lines.
"""
...
def read_xml(response: httpx2.Response) -> Element:
    ...
    Tindeerr calls are plex.tv's own, over TLS: it resolves no external entity and no
