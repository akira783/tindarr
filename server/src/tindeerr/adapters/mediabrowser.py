...
(``tindeerr.adapters.jellyfin``, ``tindeerr.adapters.emby``): the product name in
...
- ``Version`` is a **constant**, not the Tindeerr version: Emby identifies a device by
...
CLIENT_NAME: Final = "Tindeerr"
DEVICE_NAME: Final = "Tindeerr server"
CLIENT_VERSION: Final = "1"
#: Used when the connector has no install id yet (only reachable in tests).
_FALLBACK_DEVICE_ID: Final = "tindeerr"
...
class MediaBrowserServer:
    ...
    async def test(self) -> ConnectionCheck:
        ...
        if self._product_kind(info) != self.kind or not self._version_supported(info):
            ...
            # version Tindeerr supports": one problem code, ``media_server_unsupported``.
    ...
    def _maybe_user(self, row: Mapping[str, Any]) -> MediaUser | None:
        try:
            ...
            logger.warning("skipped a media server user Tindeerr cannot read", extra=self._log())
            return None
