...
3. **open the session**, which links or creates the Tindeerr user, re-reads their
...
Step 2 differs per method (the brokered ones live in ``tindeerr.auth.brokered``); steps
...
**The per-username cap** deserves its own note. Tindeerr forwards password attempts to
...
per quarter of an hour; beyond that Tindeerr answers ``429`` **without calling it**. It
...
from tindeerr.auth.mediaserver import MediaServerConnector, MediaServerSettings
from tindeerr.auth.methods import AuthMethod, password_allowed
from tindeerr.auth.ratelimit import RateLimits
from tindeerr.auth.sessions import CookieGrant, MobileGrant, SessionService
from tindeerr.auth.setup import SetupService
from tindeerr.auth.tokens import token_hash
...
class SignInService:
    ...
    async def server_name(self) -> str:
        ...
        return name if isinstance(name, str) and name else "Tindeerr"
