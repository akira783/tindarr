...
secure context, so the prefix still works), and with ``TINDEERR_ALLOW_HTTP_CONSOLE``
...
from tindeerr.api.context import RequestContext
from tindeerr.auth.errors import https_required
from tindeerr.storage.sessions import SessionKind
...
SECURE_NAMES: Final = CookieNames(
    session="__Host-tindeerr_session",
    setup="__Host-tindeerr_setup",
    preauth="__Host-tindeerr_preauth",
)
PLAIN_NAMES: Final = CookieNames(
    session="tindeerr_session", setup="tindeerr_setup", preauth="tindeerr_preauth"
)
