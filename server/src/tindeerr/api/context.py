...
- **Client address.** The TCP peer, unless it is in ``TINDEERR_TRUSTED_PROXIES``: then
  ...
  also get one warning per peer and per hour ("add it to TINDEERR_TRUSTED_PROXIES?"),
...
- **Host.** An IP literal, ``localhost``, a name in ``TINDEERR_ALLOWED_HOSTS`` or the
...
from tindeerr.api.errors import problem_response
from tindeerr.auth.errors import host_not_allowed
...
CONTEXT_SCOPE_KEY: Final = "tindeerr.context"
...
class HostPolicy:
    ...
    ``TINDEERR_ALLOWED_HOSTS`` is fixed at startup; the host of ``public_url`` is set
    when it is read from the settings and whenever it changes (step 2c).
    """
    ...
    def set_public_url(self, public_url: str | None) -> None:
        ...
        if public_url is None:
            self._public_url_host = None
            return
        _, _, authority = public_url.partition("://")
        host = parse_host(authority)
        self._public_url_host = None if host is None else host.host
...
class RequestContextMiddleware:
    ...
    def _warn_about_forwarded_headers(self, peer: IPAddress, headers: Headers) -> None:
        ...
        self._warn(
            ...
            "your reverse proxy, add it to TINDEERR_TRUSTED_PROXIES",
        )
