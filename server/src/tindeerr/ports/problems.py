...
An adapter knows nothing about ``tindeerr.auth``, but a failed sign-in must answer the
same way wherever it was detected. These constructors are that shared vocabulary: the
adapters raise them, ``tindeerr.auth`` re-exports the ones it also raises itself
(``tindeerr.auth.errors``), and the API layer only renders them.
...
def media_server_changed() -> ProblemError:
    ...
    return ProblemError(
        HTTPStatus.SERVICE_UNAVAILABLE,
        "media_server_changed",
        "This is not the media server Tindeerr was set up with; an administrator must "
        "check the connector.",
    )
