...
comes back twice, is decided in ``tindeerr.auth``. As in ``tindeerr.storage.users``,
...
from tindeerr.storage.tables import pairings, refresh_tokens, sessions
...
def new_session(
    ...
    *,
    ...
    """Build a session row (not stored yet). ``tindeerr.auth`` owns the lifetimes."""
    return Session(
        id=new_id(),
        kind=kind,
        user_id=user_id,
