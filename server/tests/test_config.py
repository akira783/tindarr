...
from tindeerr.core.config import ConfigError, ServerConfig, load_config, read_env
...
def test_reads_prefixed_environment(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("TINDEERR_API_DOCS", "true")
    monkeypatch.setenv("TINDEERR_LOG_LEVEL", "debug")
    monkeypatch.setenv("TINDEERR_DATA_DIR", "/srv/tindeerr")
    config = load_config()
    assert config.port == 9000
    assert config.api_docs is True
    assert config.log_level == "DEBUG"
    assert config.data_dir == Path("/srv/tindeerr")
...
def test_file_variant_is_read_without_trailing_newline(
    ...
    monkeypatch.setenv("TINDEERR_SECRET_KEY_FILE", str(secret_file))
    config = load_config()
...
def test_file_wins_over_plain_variable(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setenv("TINDEERR_PORT", "9000")
    monkeypatch.setenv("TINDEERR_PORT_FILE", str(port_file))
    assert load_config().port == 9001


def test_empty_values_count_as_unset(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("TINDEERR_SECRET_KEY", "")
    monkeypatch.setenv("TINDEERR_PORT_FILE", "")
    config = load_config()
    assert config.secret_key is None
    assert config.port == 8787
...
def test_unreadable_file_is_a_config_error(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.setenv("TINDEERR_SECRET_KEY_FILE", str(tmp_path / "missing"))
    with pytest.raises(ConfigError, match="TINDEERR_SECRET_KEY_FILE: cannot read"):
        load_config()


def test_read_env_accepts_an_explicit_mapping() -> None:
    assert read_env("server_name", {"TINDEERR_SERVER_NAME": "Home"}) == "Home"
    assert read_env("server_name", {}) is None


def test_invalid_values_never_echo_the_input(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("TINDEERR_PORT", "hunter2-not-a-port")
    with pytest.raises(ConfigError) as caught:
        load_config()
    assert "TINDEERR_PORT" in str(caught.value)
    assert "hunter2" not in str(caught.value)


def test_unknown_keyword_is_rejected() -> None:
    with pytest.raises(ConfigError, match="TINDEERR_NOPE"):
        load_config(nope=1)


def test_trusted_proxies_accept_addresses_and_networks(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("TINDEERR_TRUSTED_PROXIES", "10.0.0.1, 172.16.0.0/12,,fd00::/8")
...
def test_trusted_proxies_reject_garbage(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("TINDEERR_TRUSTED_PROXIES", "10.0.0.1,proxy.lan")
    with pytest.raises(ConfigError, match="TINDEERR_TRUSTED_PROXIES"):
        load_config()
